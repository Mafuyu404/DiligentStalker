...
//        return new BlockPos(center.x, instance.getY() + y, center.z);
    ...
    private int modifyChunkX(int original) {
        return SectionPos.posToSectionCoord((int) targetCenterX);
    }
    ...
    @ModifyVariable(
            ...
            at = @At(
                    value = "INVOKE",
                    target = "Lnet/minecraft/core/SectionPos;posToSectionCoord(I)I",
                    ordinal = 2
            ),
            ordinal = 2
    )
    private int modifyChunkZ(int original) {
        return SectionPos.posToSectionCoord((int) targetCenterZ);
    }
}
