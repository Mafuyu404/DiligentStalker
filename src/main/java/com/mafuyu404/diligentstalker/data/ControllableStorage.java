...
public class ControllableStorage implements IControllableStorage, INBTSerializable<CompoundTag> {
    ...
    public static final Codec<ControllableStorage> CODEC = RecordCodecBuilder.create(instance ->
            instance.group(
                    Codec.INT.fieldOf("fuel").forGetter(ControllableStorage::getFuel),
                    Codec.INT.fieldOf("maxFuel").forGetter(ControllableStorage::getMaxFuel),
                    Codec.STRING.fieldOf("cameraState").forGetter(ControllableStorage::getCameraState),
                    Codec.INT.fieldOf("signalRadius").forGetter(ControllableStorage::getSignalRadius),
                    Codec.BOOL.fieldOf("actionControl").forGetter(ControllableStorage::isActionControlling)
            ).apply(instance, ControllableStorage::new)
    );

    public static final StreamCodec<FriendlyByteBuf, ControllableStorage> STREAM_CODEC = StreamCodec.composite(
            ByteBufCodecs.INT, ControllableStorage::getFuel,
            ByteBufCodecs.INT, ControllableStorage::getMaxFuel,
            ByteBufCodecs.STRING_UTF8, ControllableStorage::getCameraState,
            ByteBufCodecs.INT, ControllableStorage::getSignalRadius,
            ByteBufCodecs.BOOL, ControllableStorage::isActionControlling,
            ControllableStorage::new
    );
